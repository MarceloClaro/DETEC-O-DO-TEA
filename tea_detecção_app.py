...
Modelo CNN: Tea_CNN_Classifier.h5
"""


# Pacotes principais
import streamlit as st
st.set_page_config(page_title="Ferramenta de Detecção do TEA", page_icon="tea.jpg", layout='centered', initial_sidebar_state='auto') # Configurações da página
...
def main(): 
	...
	<h1 style="color:yellow">Detecção do TEA</h1>
	</div>
	"""
# Título da página
	st.markdown(html_templ,unsafe_allow_html=True)
	st.write("Uma proposta simples para o diagnóstico do TEA com tecnologia Deep Learning e Streamlit")
	# Menu de navegação
	st.sidebar.image("tea.jpg",width=300)
	# Menu lateral
	image_file = st.sidebar.file_uploader("Carregar uma imagem facial (jpg, png or jpeg)",type=['jpg','png','jpeg'])
	# CARREGAR IMAGEM
	if image_file is not None:
		...
		# Diagnóstico de Covid-19 por radiografia de tórax 
			...
			if st.sidebar.button("Diagnóstico"):
				...
				st.text("TEA em escala de cinza")
				...
				model = tf.keras.models.load_model('./models/Tea_CNN_Classifier.h5')
				...
				if Diagnóstico == 0:
					st.sidebar.success("Diagnóstico: NÃO TEA (Probabilidade: %.2f%%)" % (probability_no_cov))
				else:
					st.sidebar.error("Diagnóstico: TEA (Probabilidade %.2f%%)" % (probability_cov))
				# Imagem de saída
